...
public class ServiceInfoImpl extends ServiceInfo implements DNSListener {
	...
	/**
	 * Read data bytes as a UTF stream. Add curmax, if is a length, we will not
	 * apped it into the buffer by Sin 2016-01-15
	 */
	String readUTF(byte data[], int off, int len) {
		...
		for (int end = off + len; off < end;) {
			...
			if (0 == curmax) {
				curmax = ch;
				// Log.d("---serviceinfoimpl---", "-------> " + curmax);
			} else {
				curmax--;
				buf.append((char) ch);
				// Log.d("---serviceinfoimpl---", buf.toString());
			}
		}
		return buf.toString();
	}
			...
			case DNSConstants.TYPE_SRV:
				if (rec.name.equals(getQualifiedName())) {
					...
					updateRecord(
							jmdns,
							now,
							(DNSRecord) jmdns.getCache().get(server,
	...
	public void addAnswers(DNSOutgoing out, int ttl, HostInfo localHost)
			throws IOException {
		out.addAnswer(new Pointer(type, DNSConstants.TYPE_PTR,
				DNSConstants.CLASS_IN, ttl, getQualifiedName()), 0);
		out.addAnswer(new Service(getQualifiedName(), DNSConstants.TYPE_SRV,
				DNSConstants.CLASS_IN | DNSConstants.CLASS_UNIQUE, ttl,
				priority, weight, port, localHost.getName()), 0);
