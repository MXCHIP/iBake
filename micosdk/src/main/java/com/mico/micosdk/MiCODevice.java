...
public class MiCODevice {
	 ...
	 * @param easylinkcb
	 */
	public void startEasyLink(String ssid, String password, int runSecond, EasyLinkCallBack easylinkcb) {
		if(0 == runSecond)
			runSecond = 60000 * 10;
		if (comfunc.checkPara(ssid, password) && (runSecond > 0)) {
			if(null != mContext)
				micoeasylink.startEasyLink(mContext, ssid, password, runSecond, easylinkcb);
			else{
